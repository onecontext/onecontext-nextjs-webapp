...
import {BookmarkSquareIcon, BookOpenIcon, LightBulbIcon, ShareIcon} from "@heroicons/react/24/outline"; // `rehype-katex` does not import the CSS for you


export default function Index() {

    const { messages,  reload, setMessages } = useChat({ "api": "/api/completion"});
    const [activeVideo, setActiveVideo] = useState<undefined|number>(undefined)
    ...
    const augmentedSubmit = async (event: any) => {
        ...
        // if you send ALL the chunks back from contextChat (ss function, but basically req) then bandwidth badness
        // but, if you leverage the AI state update, then you get jsondiffpatch for free
    }

    const playVideo = (index: number | null): void => {
        if (index !== null) {
                ...
                <>
                    <div className="flex flex-col flex-1">
                        ...
                        <div ref={messagesEndRef}
                            ...
                            {/*jsut give me the last user message*/}
                            ...
                            <div>
                                ...
                                <div ref={sourcesRef} className={"flex flex-col items-center"}>
                                    {aiState?.context.length > 0 && (lastContextBackTime > lastMessageDispatchTime) ?
                                      <SourcesSidebar contextSource={aiState?.context} activeVideo={activeVideo}
                            ...
                            <div>
                                <div className={'flex flex-row mt-4'}>
                                    <LightBulbIcon
                                      className="h-8 w-8 text-dorange-700 transform transition-transform duration-500 hover:animate-pulse "/>
                                    <h1 className="text-2xl font-tilt text-dorange-700 items-start m-2">Extras</h1>
                                </div>
                                <div ref={sourcesRef} className={"flex flex-col mt-4 items-center"}>
                                    {aiState?.context.length > 0 && (lastContextBackTime > lastMessageDispatchTime) ?
                                      <div className={"grid grid-cols-3 flex-items justify-between gap-4 w-4/5"}>
                                          <div
                                            className={"hover:bg-dpink-600 col-span-1 flex flex-row px-2 border border-dsand-100 rounded-lg p-2"}>
                                              <BookmarkSquareIcon
                                                className="h-6 w-6 mr-2 text-neutral-900 transform transition-transform duration-500 hover:animate-pulse "/>
                                              <button className={"col-span-1 text-neutral-900"}>
                                                  Save
                                              </button>
                                          </div>
                                          <div
                                            className={"hover:bg-dblue-200 dcol-span-1 flex flex-row px-2 border border-dsand-100 rounded-lg p-2"}>
                                              <ShareIcon
                                                className="h-6 w-6 mr-2 text-neutral-900 transform transition-transform duration-500 hover:animate-pulse "/>
                                              <button className={"col-span-1 text-neutral-900"}>
                                                  Share
                                          ...
                                          <div
                                            className={"hover:bg-dorange-600 col-span-1 flex flex-row px-2 border border-dsand-100 rounded-lg p-2"}>
                                              <BookOpenIcon
                                                className="h-6 w-6 mr-2 text-neutral-900 transform transition-transform duration-500 hover:animate-pulse "/>
                                              <button className={"col-span-1 text-neutral-900"}>
                                                  Generate Quiz
